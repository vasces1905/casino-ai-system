...
def generate_features():
    try:
        ...
        slot_path = os.path.join(data_dir, "final_slot_log_100.xml")
        tito_path = os.path.join(data_dir, "final_tito_log_matched.csv")
        crm_path = os.path.join(data_dir, "crm_profiles_100.json")
        heatmap_path = os.path.join(data_dir, "heatmap_100_customers.csv")
...
### DEBUG endpoint - Verifies Slot & TITO matches
@app.get("/debug")
def debug_merge():
    try:
